...
function expireOldNotifications(): void {
...
export function notifyNextInQueue(toolId: number): Waitlist | null {
  ...
  console.log(`[排队通知] 工具 ${nextItem.toolName} 已归还，通知排队用户 ${nextItem.userName} (${nextItem.userPhone})，请在24小时内取件`);
...
router.post('/', authMiddleware, (req: Request, res: Response) => {
  try {
    ...
    if (tool.stock > 0) {
      res.status(400).json({ success: false, error: '工具当前有库存，请直接申请借用' });
      return;
    }
    ...
    res.json({
      success: true,
      data: waitlist,
      message: `已成功加入排队，当前第 ${queuePosition} 位`,
    });
  } catch (err) {
    res.status(500).json({ success: false, error: (err as Error).message });
  }
...
router.put('/:id/cancel', authMiddleware, (req: Request, res: Response) => {
  try {
    ...
    updateQueuePositions(waitlist.toolId);

    const updated = db.prepare('SELECT * FROM waitlist WHERE id = ?').get(waitlist.id) as Waitlist;
    res.json({ success: true, data: updated, message: '已取消排队' });
  } catch (err) {
    res.status(500).json({ success: false, error: (err as Error).message });
  }
});
...
router.put('/:id/convert-to-borrow', authMiddleware, (req: Request, res: Response) => {
  try {
    ...
    if (waitlist.pickupExpiresAt && new Date(waitlist.pickupExpiresAt).getTime() < Date.now()) {
      ...
      res.status(400).json({ success: false, error: '取件时段已过期，请重新排队' });
      return;
    }
    ...
    const borrow = db.prepare('SELECT * FROM borrows WHERE id = ?').get(borrowId) as Waitlist;
  ...
  try {
    ...
    if (notified) {
      res.json({
        success: true,
        data: notified,
        message: `已通知用户 ${notified.userName}，请在24小时内取件`,
      });
    } else {
